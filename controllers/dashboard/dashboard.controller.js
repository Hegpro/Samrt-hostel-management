...
export const getParentDashboard = async (req, res) => {
  try {
    // Parent logs in using student USN + password
...
export const getWardenDashboard = async (req, res) => {
  try {
    ...
    // Room stats (using occupants, not students)
    const rooms = await Room.find({ hostelId });

    const filled = rooms.filter((r) => r.occupants?.length > 0).length;
    const empty = rooms.filter((r) => !r.occupants || r.occupants.length === 0).length;
